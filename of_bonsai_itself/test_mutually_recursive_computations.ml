...
let%expect_test "even / odd mutual recursion (Bonsai.fix)" =
  ...
  let even odd n graph =
    Bonsai.fix n graph ~f:(fun ~recurse:_ n graph ->
      match%sub n with
      | 0 -> Bonsai.return true
      | n ->
        odd
  ...
  in
  let odd n graph =
    Bonsai.fix n graph ~f:(fun ~recurse n graph ->
      match%sub n with
      | 0 -> Bonsai.return false
      | n ->
        even
          ...
          graph)
  in
  let is_even n graph = even odd n graph in
  let var = Bonsai.Expert.Var.create 42 in
  let handle =
    Handle.create
