...
let component input (local_ graph) =
  ...
  (* The first element is meant to be synced with [input], and the second is meant to
     be synced with [input_plus_one]. *)
...
let%expect_test _ =
  ...
  in
  Handle.show handle;
  (* Here, the [on_change]s all run at the beginning, so they can't run again
     to sync with the updated [input_plus_one]. *)
  [%expect {| (1 (0 0) (0 0)) |}];
  Handle.show handle;
  [%expect {| (1 (0 1) (0 1)) |}];
  Bonsai.Expert.Var.set var 1;
  Handle.show handle;
  (* Here, the two on_changes attached to [input] run first, updating [state] and
     and [input_plus_one].
